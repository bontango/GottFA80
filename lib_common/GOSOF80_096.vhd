...
-- 
-- Version for GottFA80S based on GOSOF 3.96
...
--DFcmd_par1 <= is folder number
		process (game_number)
			begin
		    case game_number is 
				 when 0 to  8 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Panthera to ForceII
				 when 		9 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111110"; DFcmd_par1 <= X"00"; --Pink Panther Folder 9?
				 when 	  10 => SB_type <= is_MA216; speech_ctrl <= "0000010000000001011110011110011";DFcmd_par1 <= X"0A"; --Mars w. speech
				 when      11 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Mars 'fake'
				 when 	  12 => SB_type <= is_MA216; speech_ctrl <= "0001000001100011011000110110011"; DFcmd_par1 <= X"0C"; --Volcano w. speech
				 when      13 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Volcano
				 when 	  14 => SB_type <= is_MA216; speech_ctrl <= "0000000000111111010111111111011"; DFcmd_par1 <= X"0E"; --BH w. speech
				 when      15 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --BH
				 when      16 => SB_type <= is_MA309; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --HH
				 when      17 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Eclipse
				 when 	  18 => SB_type <= is_MA216; speech_ctrl <= "0011111110100101101111111111111"; DFcmd_par1 <= X"12"; --Devils Dare w. speech
				 when      19 => SB_type <= is_MA55; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Devils Dare
				 when 	  20 => SB_type <= is_MA216; speech_ctrl <= "0000000000011111111111111111111"; DFcmd_par1 <= X"14"; --Rocky w. speech
				 when 21 to 22 => SB_type <= is_MA309; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Spirit & Punk
				 when 	  23 => SB_type <= is_MA216; speech_ctrl <= "0000011111111011011111111111011"; DFcmd_par1 <= X"17"; --Striker w. speech
				 when      24 => SB_type <= is_MA309; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Krull
				 when 	  25 => SB_type <= is_MA216; speech_ctrl <= "1111111111111111000011110111010"; DFcmd_par1 <= X"19"; --Q*Bert's Quest w. speech
				 when 26 to 29 => SB_type <= is_MA309; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Sup.Orbit to Amaz Hunt
				 when 30 to 37 => SB_type <= is_MA490; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --RackEmup o ICE Fever
				 when 38 to 39 => SB_type <= is_MA216; speech_ctrl <= "0000011111111011111111111111101";DFcmd_par1 <= X"3F"; --Caveman w. Speech
				 when 40 to 42 => SB_type <= is_MA490; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00"; --Bounty Hu to Tag Team
				 when others 	=> SB_type <= is_special; speech_ctrl <= "1111111111111111111111111111111"; DFcmd_par1 <= X"00";
			end case;	 
		end process;	
...
DFcmd_par2 <=  "000" & Sound_meta;

send_flag <= ( Sound_meta(0) or Sound_meta(1) or Sound_meta(2) or Sound_meta(3)) and not speech_ctrl(to_integer(unsigned(Sound_meta)));
