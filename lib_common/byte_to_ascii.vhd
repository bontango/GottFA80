...
use IEEE.numeric_std.all;
    ...
    entity byte_to_ascii is        
        port(
            clk_in  : in std_logic;               
				...
				dig0_ascii	: out character;
				dig1_ascii	: out character;
				dig2_ascii	: out character
            );
    end byte_to_ascii;
    ---------------------------------------------------
	...
	begin
	 ...
	 byte_to_ascii: process (clk_in, mybyte)
    begin
			if rising_edge(clk_in) then
				...
				when CONVERT =>				
					dig0_ascii <= character'val(to_integer(unsigned(dig0)) + 48);
					dig1_ascii <= character'val(to_integer(unsigned(dig1)) + 48);
					dig2_ascii <= character'val(to_integer(unsigned(dig2)) + 48);
					state <= INIT;
